...
dx=lambda/20.;
S_factor=1./sqrt(2.)*1.00;
dt=S_factor*dx/(cc);   %time step

nmax=250;         %total number of time steps
...
for n=1:nmax; %7.0*tau
  source(n)=sin(omega*n*dt);  %sin(omega*(n-delay)*dt)*exp(-((n-delay)^2/tau^2));
end
