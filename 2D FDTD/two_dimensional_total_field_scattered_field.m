%Total field scattered field 2-D  TM polarization
clear all;
%*********************************************************************
%   Constants
...
ex_inc(2:incm)=ex_inc(2:incm)-(dt/(eps_0*dx))*(hy_inc(2:incm)-hy_inc(1:incm-1));
...
%Incident field correction
    %left side
ey(is1,js1:js2)=ey(is1,js1:js2)-(dt/(eps_0*dx))*(hy_inc(is1-1));
    %right side
ey(is2,js1:js2)=ey(is2,js1:js2)-(dt/(eps_0*dx))*(hy_inc(is2));
    %bottom side
    for i=is1:is2-1
ex(i,js1)=ex(i,js1)+(dt/(eps_0*dx))*(hy_inc(i));
    end
    %top side
    for i=is1:is2-1
ex(i,js2+1)=ex(i,js2+1)-(dt/(eps_0*dx))*(hy_inc(i));
    end
% simple radiative boundary condition (buffer twice , at both end point i=1, ib
% ex(1,1:jb)=ex_low_m2;
% ex_low_m2=ex_low_m1;
...
hy_inc(1:incm)=hy_inc(1:incm)-(dt/(mu_0*dx)).*(ex_inc(2:inc)-ex_inc(1:incm));

% hy grid time updating
hz(1:ie,1:je)=da(1:ie,1:je).*hz(1:ie,1:je)...
    +db(1:ie,1:je).*(ex(1:ie,2:jb)-ex(1:ie,1:je) ...
    -ey(2:ib,1:je)+ey(1:ib-1,1:je));

% hy total-scatter field correction
    %left side
