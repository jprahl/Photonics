...
%     end
     ...
     %note mu not used = 1
      ...
      for i=1:ib-1 
        Ez(i,jb)=Ez(i,jb)+...
            dt1*material(i,j)*(Hy(i+1,jb)-Hy(i,jb))+ ...
            dt1*material(i,j)*(Hx(i,jb)-Hx(i,1)*exp(1i*Ky(k)*a)); 
            %dt1*material(i,j)*(Hx(i,jb)-Hx(i,1));
         %def boundary conditions
      end
       for j=1:jb-1
        Ez(ib,j)=Ez(ib,j)+...
            dt1*material(i,j)*(-Hx(ib,j+1)+Hx(ib,j)...
            +dt1*material(i,j)*(Hy(1,j)*exp(1i*Kx(k)*a)...
            -Hy(ib,j)));
