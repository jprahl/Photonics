...
t_max = 2^12;       %number of Field steps
...
for k=1:length(Kx)
    ...
    Hz=zeros(ib,jb);      %Initialize field arrays
    Ex=zeros(ib,jb);      %different in other code - ie and ib
    Ey=zeros(ib,jb);      %due to uncertain number of layers in current
...
%     end
     %initial condition (excitation) cmplx hrmnic fnc in a unit ccell
       ...
       for i=1:ib 
       Ex(i,1)=Ex(i,1)+dt1*material(i,1)*(Hz(i,1)-Hz(i,jb)*exp(-1i*Ky(k)*a)); %def boundary cconditions
     
       end                                  %NOTE - BC ASSUME MU = 0
       
       for j=1:jb                            % WHAT ABOUT THE HALF STEP?
       Ey(1,j)=Ey(1,j)-dt1*material(1,j)*(Hz(1,j)-Hz(ib,j)*exp(-1i*Kx(k)*a));
     ...
     %calculate magnetic component
        for j=2:jb
            for i=1:ib  
                ...
                Ex(i,j)=Ex(i,j)+dt1*material(i,j)*(Hz(i,j)-Hz(i,j-1));
        ...
        for j=1:jb
            for i=2:ib  
                ...
                Ey(i,j)=Ey(i,j)-dt1*material(i,j)*(Hz(i,j)-Hz(i-1,j));
      ...
      for i=1:ib-1 
        Hz(i,jb)=Hz(i,jb)-...
            dt1*(Ey(i+1,jb)-Ey(i,jb))- ...
            dt1*(Ex(i,jb)-Ex(i,1)*exp(1i*Ky(k)*a)); 
           
         %def boundary conditions
      end
       for j=1:jb-1
        Hz(ib,j)=Hz(ib,j)-...
            dt1*(Ey(1,j)*exp(1i*Kx(k)*a)...
            -Ey(ib,j))-...
            dt1*(-Ex(ib,j+1)+Ex(ib,j));
       end;
       
        Hz(ib,jb)=Hz(ib,jb)-...
            dt1*(Ey(1,jb)*exp(1i*Kx(k)*a)-Ey(ib,jb))+...
            dt1*(Ex(ib,1)*exp(1i*Ky(k)*a)-Ex(ib,jb));
           
     for j=1:jb-1
         for i=1:ib-1
         
             Hz(i,j)=Hz(i,j)-dt1*(Ey(i+1,j)-Ey(i,j))+...
                 dt1*(Ex(i,j+1)-Ex(i,j));
          
         end
     end
       
       Hz(isrc1,jsrc1)=Hz(isrc1,jsrc1)+esource; 
     ...
     %Read fields from several points and average them
        ...
        Field(nt)=Hz(iobs1,jobs1);
        Field02(nt)=(Hz(iobs2,jobs2));
        Field03(nt)=(Hz(iobs3,jobs3));
        Field04(nt)=(Hz(iobs4,jobs4));
        Field2(nt)=esource;
         ...
         HzDFT(iff,1)=HzDFT(iff,1)+Hz(iobs1,jobs1)*rd(iff,1);
         HzDFT(iff,2)=HzDFT(iff,2)+Hz(iobs1,jobs1)*rd(iff,2);
         end
        if(mod(nt/dt,100) ==0)
            figure(3);pcolor(Hz);shading flat;colorbar;daspect([1 1 1]);
          %  pause
         end
    end
    ...
    HzDFT(1:nfreq,1)=sqrt(HzDFT(1:nfreq,1).^2+HzDFT(1:nfreq,2).^2)./snorm(1:nfreq,1);
    figure(6);plot(omega,snorm(1:nfreq,1));
    figure(7);plot(omega,log(HzDFT(1:nfreq,1)));
    figure(8);plot(omega,(HzDFT(1:nfreq,1)));
    %pause;
   ...
   % end
    ...
    for u=2:wcountmax
        if(abs(fourier(u))/abs(fourier(u-1))>1.0001)&&...
                ...
                ((abs(fourier(u))/abs(fourier2(u)))>0.1)
    ...
    if(wcount-1>=16)
        ...
        plot(ones(1,wcount-1)*sqrt(Kx(k)^2+Ky(k)^2)*a/2/pi,...
            abs(wEigen(numK,1:wcount-1)*(a/cc)),'o','LineWidth',3);
    end
 ...
 fprintf(simulation_data, '%6.3f %13.9f\n', wEigen);
